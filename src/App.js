    ...
    switch(action.type) {
        ...
        case 'withdraw':
            return{
                ...currentState,
                balance: currentState.balance - action.payload,
                //isActive: currentState.balance === 0 ? false : currentState.isActive
            }
        case 'loan':
            if (currentState.loan > 0) return currentState.loan
            return{
                ...currentState,
                loan: action.payload,
                balance: currentState.balance + action.payload
...
export default function App() {
  ...
  return (
    <div className="App">
      ...
      <p>
        <button onClick={() => dispatch({type: 'deposit' , payload: DEPOSIT})} disabled={!isActive}>
          Deposit 150
        </button>
      </p>
      <p>
        <button onClick={() => dispatch({type: 'withdraw', payload: WITHDRAW})} disabled={!isActive}>
          Withdraw 50
        </button>
      </p>
      <p>
        <button onClick={() => dispatch({type: 'loan', payload: LOAN})} disabled={!isActive}>
